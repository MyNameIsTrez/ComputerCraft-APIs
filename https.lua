...
function getFile(...)
	local ar = arg[1]
	local str = https.get(ar.url)
	local fileLines = {}
	if ar.convertToUnicode then
		str:gsub(ar.delimiter, function(line) fileLines[#fileLines + 1] = textutils.unserialize(unicodify(line)) end)
	elseif ar.convertToUtf8 then
		str:gsub(ar.delimiter, function(line) fileLines[#fileLines + 1] = textutils.unserialize(utf8ify(line)) end)
	else
		str:gsub(ar.delimiter, function(line) fileLines[#fileLines + 1] = textutils.unserialize(line) end)
	end
	return fileLines
end
...
function setGoogleDriveStructure()
	...
	local args = { url=fileIndexUrl, delimiter=googleDriveDelimiter, convertToUnicode=true }
	googleDriveStructure = getFile(args)
end
